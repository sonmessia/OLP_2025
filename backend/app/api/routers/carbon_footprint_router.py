...
async def get_all_carbon_footprint(
    ...
    """
    Retrieve a list of CarbonFootprint entities based on query parameters.
    """
    try:
...
@router.get(
    ...
    summary="Get CarbonFootprint Entity by ID",
)
async def get_carbon_footprint_by_id(
    ...
    """
    Retrieve a single CarbonFootprint entity by its full URN.
    """
    try:
        attrs_list = attrs.split(",") if attrs else None
        return await carbon_footprint_service.get_entity_by_id(
...
@router.post("/", status_code=201, summary="Create CarbonFootprint Entity")
async def create_carbon_footprint(
    ...
    """
    try:
        orion_response = await carbon_footprint_service.create_entity(entity_data)
...
async def update_carbon_footprint_attributes(
    ...
    """
    Update one or more attributes of a CarbonFootprint entity using
    ...
    try:
        ...
        await carbon_footprint_service.update_entity_attributes(
            entity_id, payload_to_send
        )
        return
    except ValueError as e:
        raise HTTPException(status_code=422, detail=str(e))
...
@router.delete(
    "/{entity_id:path}", status_code=204, summary="Delete CarbonFootprint Entity"
)
async def delete_carbon_footprint(entity_id: str):
    """
    Delete a CarbonFootprint entity by its ID.
    """
    try:
        await carbon_footprint_service.delete_entity(entity_id)
        return
    except httpx.HTTPStatusError as e:
        raise HTTPException(
            status_code=e.response.status_code, detail=e.response.json()
