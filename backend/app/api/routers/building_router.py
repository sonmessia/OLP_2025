...
async def get_all_buildings(
    ...
    """
    Retrieve a list of Building entities based on query parameters.
    """
    try:
...
@router.get(
    ...
    summary="Get Building Entity by ID",
)
async def get_building_by_id(
    ...
    """
    Retrieve a single Building entity by its full URN.
    """
    try:
        attrs_list = attrs.split(",") if attrs else None
        return await building_service.get_entity_by_id(
...
@router.post("/", status_code=201, summary="Create Building Entity")
async def create_building(response: Response, entity_data: Dict[str, Any] = Body(...)):
    """
    Create a new Building entity.
    """
    try:
        orion_response = await building_service.create_entity(entity_data)
...
async def update_building_attributes(
    ...
    """
    Update one or more attributes of a Building entity using
    ...
    try:
        ...
        await building_service.update_entity_attributes(entity_id, payload_to_send)
        return
    except ValueError as e:
        raise HTTPException(status_code=422, detail=str(e))
    except httpx.HTTPStatusError as e:
...
@router.delete("/{entity_id:path}", status_code=204, summary="Delete Building Entity")
async def delete_building(entity_id: str):
    """
    Delete a Building entity by its ID.
    """
    try:
        await building_service.delete_entity(entity_id)
        return
    except httpx.HTTPStatusError as e:
        raise HTTPException(
            status_code=e.response.status_code, detail=e.response.json()
