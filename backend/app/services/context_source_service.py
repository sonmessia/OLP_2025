...
class ContextSourceService(BaseService):
    ...
    def create_context_source(context_source_data: Dict[str, Any]):
        ...
        response = BaseService._make_request(
            "POST", url, headers=headers, json_data=context_source_data
        )
        return response.json()

    @staticmethod
    def get_all_context_sources(
        limit: Optional[int] = None, offset: Optional[int] = None, count: bool = False
    ):
        ...
        return (
            response.json() if not count else response.headers.get("X-Total-Count", 0)
        )
