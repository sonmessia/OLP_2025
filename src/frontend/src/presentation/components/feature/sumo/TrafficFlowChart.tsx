...
export const TrafficFlowChart: React.FC = () => {
  ...
  const chartData = getChartData(dataHistoryRef.current, t);
  const options = getChartOptions(isDarkMode, t);
        ...
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center gap-4 text-gray-500 dark:text-gray-400">
            <span className="font-medium">
              {dataHistoryRef.current.length} {t("flowChart.dataPoints")}
            </span>
            {hasRealData && (
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                {t("flowChart.liveUpdating")}
              </span>
            )}
          </div>
