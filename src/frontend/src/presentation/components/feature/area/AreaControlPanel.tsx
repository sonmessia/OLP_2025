...
export const AreaControlPanel: React.FC<AreaControlPanelProps> = ({ areaName, onLog }) => {
  ...
  const scenario = TRAFFIC_LOCATIONS.find((loc) => loc.nameKey === areaName)?.id
  ...
  const handleStartSumo = async () => {
    ...
    try {
      const config = SumoModelFactory.createConfiguration(scenario, useGUI, port)
  ...
  const handleStepForward = useCallback(async () => {
    ...
    try {
      ...
      onLog?.(`⏭️ Simulation step executed at time: ${simulationState?.simulationTime || 'N/A'}`)
    } catch (error) {
    ...
    <div className="glass-card rounded-xl p-6 shadow-xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-full">
          ...
          <span className="text-sm font-medium text-blue-700 dark:text-blue-300">{areaName}</span>
        </div>
      </div>

      <SumoStatusDisplay status={displayStatus} simulationState={displayState} />
      ...
      <div className="my-4">
        <label className="flex items-center space-x-2 cursor-pointer">
          ...
          />
          <span className="text-sm text-gray-700 dark:text-gray-300">{t('useGui')}</span>
        </label>
      </div>
