...
class SumoRunner:
    ...
    def __init__(self, scenario: str = "Nga4ThuDuc", gui: bool = True):
        ...
        if not _SUMO_AVAILABLE:
            raise RuntimeError(
                "SUMO is not available. Please set SUMO_HOME environment variable."
            )
    ...
    def start(self, port: int = 8813, step_length: float = 1.0):
        ...
        try:
            ...
            sumo_cmd = [
                ...
                str(step_length),
                "--quit-on-end",
                "false",  # Don't quit when simulation ends
                "--start",
                "true",  # Auto-start simulation (for GUI)
                "--delay",
                "100",  # Delay between steps (ms) for GUI visibility
            ]
            ...
            logger.info(
                f"   Number of traffic lights: {len(traci.trafficlight.getIDList())}"
            )
    ...
    def get_traffic_state(self):
        ...
        try:
            ...
            state["avg_speed"] = (
                total_speed / total_vehicles if total_vehicles > 0 else 0
            )
            state["occupancy"] = (
                sum(d["occupancy"] for d in state["detectors"].values())
                / len(detector_ids)
                if detector_ids
                else 0
            )
        ...
        try:
            ...
            while step < duration:
                ...
                if step % 10 == 0:
                    ...
                    if state:
                        logger.info(
                            f"[{step}s] Vehicles: {state['vehicle_count']}, "
...
def main():
    ...
    parser.add_argument(
        "--gui", action="store_true", help="Use SUMO GUI (default: headless)"
    )
    parser.add_argument(
        "--duration",
        type=int,
    ...
    parser.add_argument(
        "--port", type=int, default=8813, help="TraCI port (default: 8813)"
    )
