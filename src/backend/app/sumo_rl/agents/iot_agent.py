...
class IoTAgent:
                ...
                if entity.get("type") == "TrafficLight":
                    ...
                    if force_phase_attr:
                        ...
                        if phase_index is not None and phase_index >= 0:
                            logger.info(
                                f"[IoT Agent] Received command: forcePhase={phase_index}"
                            )
                            ...
                            if self.sumo_connected and self.traci:
                                ...
                                logger.warning(
                                    "[IoT Agent] SUMO not connected - command logged only"
                                )
                                return {
                                    "applied": False,
                                    "phase": phase_index,
                                    "reason": "sumo_not_connected",
                                }
    ...
    def apply_phase(self, phase_index: int, tls_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"[IoT Agent] ✅ Applied phase {phase_index} to {actual_tls_id}"
            )
            return True
    ...
    def get_traffic_state(self, tls_id: str, detector_ids: list) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"queues": queues, "phase": phase, "connected": True}
