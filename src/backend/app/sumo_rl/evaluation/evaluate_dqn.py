...
SUMO_CONFIG_BASE = [
    "sumo",
    "-c",
    "sumo_files/Nga4ThuDuc/Nga4ThuDuc.sumocfg",
    "--step-length",
    "0.1",
    "--lateral-resolution",
    "0",
...
class TrafficMetrics:
    ...
    def update(self, step):
        ...
        queue_length = sum(
            [traci.lanearea.getLastStepHaltingNumber(det) for det in DETECTOR_IDS]
        )
    ...
    def get_summary(self):
        ...
        return {
            ...
            "combined_score": (
                ...
                + (self.total_waiting_time / max(self.step_count, 1))
                * 0.3  # Waiting time weight
            ),
        }
...
def compare_and_visualize(
    ...
    for metric in metrics_names:
        ...
        if metric == "throughput":
            improvement = (
                ((dqn_val - base_val) / base_val * 100) if base_val != 0 else 0
            )
        else:
            improvement = (
                ((base_val - dqn_val) / base_val * 100) if base_val != 0 else 0
            )

        print(
            f"{metric:<25} {base_val:<15.2f} {rand_val:<15.2f} {dqn_val:<15.2f} {improvement:>+.2f}%"
        )
    ...
    results = {
        ...
        "improvements": {
            metric: (
                (
                    (baseline_summary[metric] - dqn_summary[metric])
...
def main():
    ...
    print("=" * 60)
    print(
        f"Evaluation duration: {EVALUATION_STEPS} steps ({EVALUATION_STEPS * 0.1:.1f} seconds)"
    )

    # Check if model exists (prefer .keras format)
    model_path = (
        "dqn_model.keras" if os.path.exists("dqn_model.keras") else "dqn_model.h5"
    )
    ...
    if dqn_summary["combined_score"] == best_score:
        improvement = (
            (baseline_summary["combined_score"] - dqn_summary["combined_score"])
            / baseline_summary["combined_score"]
            * 100
        )
        ...
        print(
            f"   Overall Performance: {'EXCELLENT' if improvement > 20 else 'GOOD' if improvement > 10 else 'MODERATE'}"
        )
    else:
        print("⚠️  DQN Model needs improvement")
        if baseline_summary["combined_score"] < dqn_summary["combined_score"]:
            print(
                f"   Baseline performs better by {((dqn_summary['combined_score'] - baseline_summary['combined_score']) / baseline_summary['combined_score'] * 100):.2f}%"
            )
        print(
            "   Suggestions: Increase training steps, tune hyperparameters, or adjust reward function"
        )
...
if __name__ == "__main__":
    if not _SUMO_AVAILABLE:
        print(
            "ERROR: SUMO is not available. Please set SUMO_HOME environment variable.",
            file=sys.stderr,
        )
        sys.exit(1)
    main()
