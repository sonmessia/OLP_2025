...
def build_model(state_size, action_size):
    ...
    model.compile(
        loss="mse", optimizer=keras.optimizers.Adam(learning_rate=LEARNING_RATE)
    )
    return model
...
def main_loop():
    ...
    print(
        f"Epsilon decay: {EPSILON_START} → {EPSILON_END} over {EPSILON_DECAY_STEPS:,} steps"
    )
    print("=" * 70 + "\n")
    ...
    for step in range(TOTAL_STEPS):
        ...
        if step > 0 and step % 1000 == 0:
            ...
            print(
                f"   Buffer Utilization: {len(replay_buffer)/REPLAY_BUFFER_SIZE*100:.1f}%"
            )
            print(f"{'='*70}\n")
