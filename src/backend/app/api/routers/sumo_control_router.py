...
def start_sumo_on_host(scenario: str) -> bool:
    ...
    try:
        ...
        logger.error(
            "Please start it on the host with: python /scripts/sumo_starter_service.py"
        )
        return False
...
async def connect_to_simulation(request: ConnectSimulationRequest):
    ...
    try:
        ...
        success = traci_connector.connect(
            host=request.host, port=request.port, scenario=request.scenario
        )
...
async def simulation_step():
    ...
    try:
        ...
        return {"status": "ok", "time": sim_time, "state": state}
    ...
    try:
        ...
        for tls_id in all_tls_ids:
            smart_controllers[tls_id] = SmartTrafficController(
                tls_id=tls_id, min_green_time=10  # Minimum 10s green time
            )

        logger.info(
            f"✅ AI Traffic Control enabled for {len(smart_controllers)} traffic lights"
        )
        ...
        for tls_id, controller in smart_controllers.items():
            try:
                ...
                if best_phase != current_phase:
                    logger.info(
                        f"🚦 TLS {tls_id}: Switching {current_phase} → {best_phase}"
                    )
                    ...
                    decisions.append(
                        {
                            ...
                            "explanation": controller.get_phase_explanation(
                                current_phase
                            ),
                        }
                    )
                ...
                decisions.append({"tls_id": tls_id, "action": "error", "error": str(e)})
...
async def get_simulation_status():
    ...
    return {**info, "current_state": state}
